...
final class MapViewModel: ObservableObject {
    ...
    private static let mapPerspectiveModeDefaultsKey = "cheeseek.mapPerspectiveMode"
    private static let mapStyleModeDefaultsKey = "cheeseek.mapStyleMode"
    ...
    @Published private(set) var walkHistoryRevision = 0
    ...
    func finishWalk() async {
        ...
        do {
            ...
            let coverageSnapshot = persistCoverageLocally(from: session)
            publishFinishedSession(session)
            if let coverageSnapshot {
                Task { await syncCoverageSnapshot(coverageSnapshot.coveredFeatureIDs, for: coverageSnapshot.profile) }
            }
        } catch {
            session.syncStatus = .failed
            syncStatusText = session.syncStatus.label
            permissionMessage = "Could not save this walk to the backend. Check the server and try again."
            activeSession = session
            walkState = .paused
            updateExplorationState()
            refreshMapData()
    ...
    private func loadStoredSessions() async {
        do {
            ...
            storedSessions = try await backendSyncService.fetchWalkSessions(userId: profileID)
    ...
    private func upsertStoredSession(_ session: WalkSession) {
        ...
        storedSessions.sort(by: { $0.startedAt > $1.startedAt })
    }

    private func publishFinishedSession(_ session: WalkSession) {
        activeSession = nil
        activeCoverageSideIDs = []
        walkState = .finished(session)
        summarySession = session
        syncStatusText = session.syncStatus.label
        walkHistoryRevision += 1
        updateMapRoutes()
        updateExplorationState()
        refreshMapData()
    }
    ...
    private func persistCoverageLocally(from session: WalkSession) -> (profile: UserProfile, coveredFeatureIDs: Set<String>)? {
    ...
    private func updatePublishedFinishedSessionIfNeeded(_ session: WalkSession) {
        guard summarySession?.id == session.id else { return }
        walkState = .finished(session)
        summarySession = session
        syncStatusText = session.syncStatus.label
    }
